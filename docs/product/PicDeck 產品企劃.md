...
| MVP | 長按照片的操作選單，與整理的審核畫面一致：寫日記、標籤、喜愛、加到相簿（子選單列出本機相簿） | 自訂 |
| MVP | 自訂標籤：長按照片或在審核畫面加標籤（免費），篩選選單可依標籤篩選（付費） | Clutter |
| MVP | 標籤名稱不可重複：比對時去掉前後空白，且忽略大小寫與全形半形差異，重複時「新增」停用並顯示提示；改名同樣擋重複 | 自訂 |
| MVP | 共用的圖示選擇器（參考 Notion）：表情符號與系統圖示兩頁、關鍵字搜尋、隨機、移除、最近使用（只列當頁類型）；系統圖示那頁可以挑內建色或自訂色。標籤符號與日記心情都用同一個 | Notion |
| MVP | 標籤只有一張表單：圖示、名稱、紀念日。建立與編輯共用，從加標籤或管理頁的「建立標籤」按鈕開啟 | 自訂 |
| MVP | 照片右側的拖拉軸：平常停在中下方方便單手操作，按住拖動時顯示年份刻度與目前畫面的日期區間，放開後停留一下再收起 | 相冊助手、iOS 照片 |
| MVP | 篩選選單把標籤收成下一層；標籤超過十個時只列常用的，其餘進「全部標籤」的可搜尋清單 | 自訂 |
| MVP | 日記分頁跟著篩選走，只列出有照片落在篩選結果裡的日記 | 自訂 |
...
| MVP | 多選：全部與時間軸右上角有選擇按鈕，勾選後可批次寫日記、加標籤、加入喜愛、加到相簿 | 相冊助手 |
| MVP | 批次寫日記只在勾選的照片同屬一天時出現，跨日會隱藏；批次加入喜愛會跳過原本就已是喜愛的照片 | 自訂 |
...
| MVP | 加到相簿跟加標籤同一套邏輯：最上面「建立相簿或資料夾」，下面是相簿清單，已在裡面的打勾，點一下加入、再點移出；長按選單與多選都用同一張表單 | 自訂 |
| MVP | 建立的相簿與資料夾都在系統照片裡：可以建資料夾、在資料夾底下建相簿，資料夾裡還能再放資料夾，清單照階層顯示並可收合 | 自訂 |
...
- **PhotoKit**：直接讀寫系統相簿，所有刪除/歸檔/喜愛動作同步回 iOS，App 本身不留實體資料副本（沿用 Slidebox 與相冊助手的作法，避免用戶資料被鎖定）。
...
- **分頁**：首頁／照片／整理／更多。原「資料夾」分頁併入 整理 → 資料夾及相簿。
- **照片分頁標題即標籤切換**：點標題選「所有項目」或某個標籤；右上角篩選只剩媒體類型。
- **首頁**：紀念日／孩子卡片（設了起算日的標籤）、**釘選在首頁的標籤**（標籤表單的「釘選在首頁」）、那年今天。沒釘的標籤只出現在照片分頁的篩選；備註不顯示在首頁。
- **收藏**：點首頁的標籤進入，列出該標籤的照片與備註；上方是同時帶有的其他標籤，可多選做二次篩選，也能搜尋備註文字。用途例：截圖想去的餐廳，標 #美食 #燒肉，備註寫店名與營業時間。備註沒有待辦／完成狀態。
- **長按照片選單**：編輯時間（有備註／標籤／系統修改才顯示）、分隔線、寫日記、備註、標籤、喜愛（移出喜愛）、加到相簿、刪除（紅字、X 圖示，與整理一致）、分隔線、選取（進入多選並選這張）。
- **字級**：整個 App 統一 `dynamicTypeSize(.small)`。系統長按選單的字不受此設定影響。
