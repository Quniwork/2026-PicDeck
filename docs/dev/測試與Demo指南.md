...
| `testIconPickerAndLibraryManager` | 圖示選擇器、配色盤、標籤表單、管理相簿 | 95 秒 |
| `testScrubber` | 右側拖拉軸的年份刻度與日期區間 | 32 秒 |
...
**只驗這次改到的部分。** 沒有動到的畫面不用重跑，用 `-only-testing` 指定就好；只有在改到共用元件、確實會影響其他畫面時，才多跑受影響的那幾個。
...
| 共用圖示選擇器：表情符號／系統圖示兩頁、搜尋、隨機、移除、最近使用（分頁各自獨立） | 完成 |
| 圖示配色盤：十二個內建色點 + 自訂顏色，顯示色點不顯示文字 | 完成 |
| 標籤表單：建立與編輯共用一張，含圖示、名稱、紀念日 | 完成 |
| 照片右側拖拉軸：把手停在中下方，拖動顯示年份刻度與畫面日期區間 | 完成 |
| 篩選選單的標籤收成下一層，超過十個時提供可搜尋的「全部標籤」 | 完成 |
| 日記分頁跟著篩選走 | 完成 |
